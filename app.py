...
# entrar no site do https://tjrj.pje.jus.br/1g/ConsultaPublica/listView.seam
...
dropdownn_estados = driver.find_element(
    By.XPATH, "//select[@id='fPP:Decoration:estadoComboOAB']")
opcoes_estados = Select(dropdownn_estados)
...
#Guardar tudo no excel, separados por processo
    ...
    try:
        ...
        for index, linha in enumerate(pagina_processo.iter_rows(min_row=2, max_row=len(lista_movimentacoes), min_col=3, max_col=3)):
            for celula in linha:
                celula.value = lista_movimentacoes[list]
        workbook.save("dados.xlsx")
        driver.close()
        sleep(5)
        driver.switch_to.window(driver.window_handles[0])
